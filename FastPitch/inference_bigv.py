...
def main():
    ...
    log_fpath = args.log_file or str(Path('./log/', 'nvlog_infer.json'))
    ...
    print(args.hifigan, args.waveglow, args.fastpitch)
    # elif args.bigvgan is not None:
    #     voc_name = 'bigvgan'
    #     vocoder, voc_train_setup = _load_pyt_or_ts_model('BigV-GAN',
    #                                                      args.bigvgan)
    #
    #     if args.denoising_strength > 0.0:
    #         denoiser = Denoiser(vocoder, win_length=args.win_length).to(device)
    #
    #     if args.torch_tensorrt:
    #         vocoder = models.convert_ts_to_trt('BigV-GAN', vocoder, parser,
    #                                            args.amp, unk_args)
    #
    #     def generate_audio(mel):
    #         audios = vocoder(mel).float()
            ...
            if generator is None:
                ...
                if args.save_mels:
                    for i, mel_ in enumerate(mel):
                        ...
                        np.save(mel_path, mel_[:, :mel_lens[i].item()].unsqueeze(0).cpu().numpy())
